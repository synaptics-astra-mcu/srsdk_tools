...
if system == "windows":
    ...
    if "x86_64" in machine or "amd64" in machine:
        GENX_EXE_PATH = Path.cwd().joinpath("genx_main_linux_x86")
    elif "aarch64" in machine or "arm64" in machine:
        GENX_EXE_PATH = Path.cwd().joinpath("genx_main_linux_aarch64")
    else:
        raise RuntimeError(f"Unsupported Linux architecture: {machine}")
elif system == "darwin":  # macOS
    if "x86_64" in machine:
        GENX_EXE_PATH = Path.cwd().joinpath("genx_main_mac_x86")
    elif "arm64" in machine:
        GENX_EXE_PATH = Path.cwd().joinpath("genx_main_mac_arm64")
    else:
...
SPK_PRIVKEY                     = "SPK_Private_Key.pem"
SPK_PUBKEY                      = "SPK_Public_Key.pem"
SPK_HASH                        = "SPK_SHA512.bin"
SPK_SHA256_in                   = "SPK_SHA256_in.bin"
SPK_SHA256_out                  = "SPK_SHA256_out.bin"
SPK_SIGNATURE                   = "SPK_Signature.bin"
...
# opcodes list
#define OPCODE_READ_VERSION             (0x00)
#define OPCODE_DWNLD_KEYS               (0x01)
#define OPCODE_DWNLD_SPK_IMG            (0x02)
#define OPCODE_READ_SPK_VERSION         (0x03)
#define OPCODE_DWNLD_BL_IMG             (0x04)

#OPCODE Defines
OPCODE_READ_VERSION				= [0x00]
OPCODE_DWNLD_KEYS				= [0x01]
OPCODE_DWNLD_SPK_IMG            = [0x02]
OPCODE_READ_SPK_VERSION         = [0x03]
OPCODE_DWNLD_APBL_IMG           = [0x04]
OPCODE_DWNLD_APP_IMG            = [0x06]
OPCODE_RUN_FW_IMG				= [0x07]
OPCODE_FLASH_SRV				= [0x08]
OPCODE_DWNLD_SCR_IMG_DEC_IP		= [0x09]
OPCODE_DWNLD_SCR_IMG_DEC_NOT_IP = [0x0A]
...
CIPHER_20_35                 = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
IMG_TYPE_APBL_8_11           = [0x11, 0x00, 0x00, 0x00] #Image type for APBL
IMG_TYPE_APP_8_11            = [0x12, 0x00, 0x00, 0x00] #Image type for APP
IMG_VER_APBL_12_15           = [0x00, 0x00, 0x00, 0x00] #Image version for APBL
IMG_VER_APP_12_15            = [0x00, 0x00, 0x00, 0x00] #Image version for APP
SEG_ID_64_67                 = [0x78, 0x56, 0x34, 0x12] #Segment ID for APBL
VER_68_71                    = [0x00, 0x00, 0x00, 0x00] #Version for APBL
PRODUCTION_FLAG              = [0x00, 0x00, 0x00, 0x00] #Production flag
RFU_72_75                    = [0x00, 0x00, 0x00, 0x00] #Reserved for future use for APBL
DESTINATION_ADDRESS_APBL     = [0x00, 0xb0, 0x02, 0x38] #Destination address APBL
DESTINATION_ADDRESS_XSPI_APP = [0x00, 0x20, 0x07, 0x38] #Destination address APP
DESTINATION_ADDRESS_APP      = [0x00, 0x00, 0x00, 0x32] #Destination address APP
BOOTLOADER_ENTRY_POINT       = [0x00, 0xb0, 0x02, 0x38] #Bootloader entry point APBL
DESTINATION_HEADER_ADDRESS_APP = [0x00, 0xA0, 0x00, 0x30] #Address in header for APP
# APP_ENTRY_POINT            = [0x00, 0x80, 0x04, 0x38] #Bootloader entry point APP
zero_pad_24_bytes            = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
...
SECURITY_HEADER                 = (int)(116) #size in bytes
...
AES_KEY_STR                     = ('sabrea0encryptionkey0123456789ab').encode('utf-8')
AES_INITIAL_COUNTER_NONCE       = b'0000000000000000'
NONCE_SIZE_BITS                 = 128
NUM_OF_IV                       = 4
FFFFFFFF                        = "FFFFFFFF"
checksum                        = 0
...
output_fw_m52_secured_host      = ""
output_fw_m52_secured_flash     = ""

output_model_flash              = ""

output_fw_b_secured             = ""
output_fw_m52_b_secured_flash   = ""
output_fw_m52_secured           = ""
