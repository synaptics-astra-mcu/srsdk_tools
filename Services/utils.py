...
def run_Genx_secured(dict, payload_length, input_bin_file, output_bin_file):
    ...
    #create cmd line
    cmd = [str(image_gen_config.GENX_EXE_PATH), '-t', current_image, '-o', output_bin_file,
                '-k', dict["SCGK_path"], '-K', dict["IV_path"], '-n', dict["Priv_Key_Path"],
                '-s', dict["SegID_Value"], '-r', dict["Version_Value"], '-x', str(image_gen_config.EXTRAS_BIN_PATH),
    ...
    try:
        ...
        # handle the exception   
        print_err('run_Genx_secured ERROR: An exception occurred:', an_exception= str(error))
    end_time = time.time()
    execution_time = end_time - start_time
    image_gen_config.total_run_time +=  execution_time
